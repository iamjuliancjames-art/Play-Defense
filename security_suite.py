# =============================================================================
# Play Defense Device Security Suite – v1.0 (Feb 2026)
# Locally hosted full device security suite using Play Defense logic + Windows Defender integration
# MIT License – https://github.com/iamjuliancjames-art/Play-Defense
#
# FEATURES:
# - Runs Windows Defender scans via MpCmdRun.exe (local, no cloud)
# - Applies Play Defense fractal/nuance/novelty logic to scan logs/files
# - Threat detection, lures, quarantine (archives suspect logs)
# - Eternal memory for scan history
# - GUI for scan, analysis, status
# - No new dependencies – uses stdlib + subprocess for Defender
# =============================================================================
...
# Log file for eternal scan history
LOG_FILE = "eternal_scan_log.jsonl"

# Tuning knobs (default Gateway)
SIM_THRESHOLD = 0.30
NOVELTY_GATE = 0.65
...
# =============================================================================
class RoomStore:
    def __init__(self, max_rooms: int = 1000000):
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            textarea, input { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; }
            #output { white-space: pre-wrap; background: #f8f9fa; padding: 15px; border-radius: 8px; min-height: 200px; overflow-y: auto; }
        ...
        <script>
            ...
            function doSearch() {
                ...
                .then(data => {
                    ...
                    if (data.safe === false) {
                        out += '<p style="color:red">Threat: ' + data.tier + ' - ' + data.reason + '</p>';
                    } else {
...
@app.route('/autotune_news', methods=['GET'])
def autotune_news():
    cs.autotune_from_news()
    return jsonify({"risk": 0.5, "level": cs.current_level})  # Placeholder - update with real score
